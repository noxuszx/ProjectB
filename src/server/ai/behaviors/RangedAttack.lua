...
local ProjectileService =
	require(ServerScriptService.Server.services.ProjectileService)
...
function RangedAttack.new(targetPlayer)
	...
	-- Throttle how often we reconsider whether we are still in optimal range.
...
function RangedAttack:enter(creature)
	...
	if AIConfig.Debug.LogBehaviorChanges then
		...
		print(
			"[RangedAttack] " .. creature.creatureType .. " attacking " .. targetName
		)
	end
end
...
function RangedAttack:update(creature, deltaTime)
	...
	-- Re-evaluate range only every rangeCheckRate seconds to avoid jitter.
	local currentTime = os.clock()
	if currentTime - self.lastRangeCheck >= self.rangeCheckRate then
		self.lastRangeCheck = currentTime
		-- If target moved too far from optimal range, switch back to chasing (wider tolerance)
	...
	-- IK Control handles targeting automatically - no need for manual faceTarget
	...
	if AIConfig.Debug.LogBehaviorChanges and math.random() < 0.01 then
		print(
			"[RangedAttack] " .. creature.creatureType .. " attacking: distance=" .. string.format(
				"%.1f",
				distance
			) .. ", LOS=" .. tostring(self.hasLOS)
		)
	end
end
...
function RangedAttack:checkLineOfSight(creature, targetPosition)
	local origin =
		ProjectileService.getMuzzlePosition and ProjectileService.getMuzzlePosition(
			creature
...
function RangedAttack:fireProjectile(creature, targetPosition)
	...
	local weaponName =
		creature.getWeaponName and creature:getWeaponName() or "SkeletonArrow"
	...
	-- Update creature's fire cooldown
	...
	if AIConfig.Debug.LogBehaviorChanges then
		...
		print(
			"[RangedAttack] " .. creature.creatureType .. " fired at " .. targetName .. " - " .. hitInfo
		)
	end
end
...
function RangedAttack:stopAttacking(creature, reason)
	if AIConfig.Debug.LogBehaviorChanges then
		print(
			"[RangedAttack] " .. creature.creatureType .. " stopping attack: " .. reason
		)
	end
